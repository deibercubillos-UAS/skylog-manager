...
- **Subida de imágenes/documentos (proxy server-side)**: `FileUpload.js` (bucket `documents`) y `FleetImageUpload.js` (`fleet-images`) suben via **`POST /api/storage/upload`** (multipart, mismo origen → el servidor hace `storagePut`). Esto evita CORS/URL-prefirmada y es inmune a bloqueos de extensiones del navegador sobre `cloudflarestorage.com`. **Límite 4 MB** (cuerpo serverless Vercel = 4.5 MB). Sigue existiendo `POST /api/storage/sign-upload` (presigned PUT directo) para archivos grandes / otros flujos (p.ej. `AddMaintenancePanel`).
