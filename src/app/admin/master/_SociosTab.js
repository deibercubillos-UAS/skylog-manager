                ...
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-500 font-bold mt-1">
                      Comisión {p.commission_pct}% · Cupos {p.free_seats_limit ?? '∞'} (usados {p.free_seats_used}) · {p.free_days} días gratis · {p.member_count} miembro(s)
                    </p>
                  ...
                  <div className="flex gap-2 shrink-0">
                    <button onClick={() => startEdit(p)} className="px-3 py-2 bg-slate-100 rounded-lg text-xs font-black uppercase text-slate-600 hover:bg-slate-200">Editar</button>
                    <button onClick={() => toggleStatus(p)} className={`px-3 py-2 rounded-lg text-xs font-black uppercase ${p.status === 'activo' ? 'bg-red-50 text-red-600 hover:bg-red-100' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100'}`}>
