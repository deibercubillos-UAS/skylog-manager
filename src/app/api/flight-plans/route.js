...
export async function POST(request) {
  try {
    ...
    // El piloto independiente siempre tiene role='admin' (nunca 'piloto') — por lo
    // tanto role==='piloto' aquí siempre es un miembro de una organización, que ya
    // no planea sus propios vuelos: solo vuela lo que le programaron (Programación).
    if (role === 'piloto') {
      return NextResponse.json({ error: 'Los pilotos de una organización no planean vuelos — solo vuelan lo que se les programa.' }, { status: 403 });
    }
