...
export default function FleetPage() {
  ...
  if (loading) return <div className="p-20 text-center animate-pulse font-black uppercase text-slate-300">Sincronizando Inventario...</div>;

  return (
    <div className="space-y-16 text-left animate-in fade-in duration-500 pb-20">
      <section className="space-y-6">
        <header className="flex justify-between items-end border-b border-slate-200 pb-4 text-left">
          <div>
          ...
          <button onClick={() => setActivePanel('drone')} className="bg-[#1A202C] text-white px-6 py-2.5 rounded-xl font-black text-[10px] uppercase shadow-lg hover:bg-slate-800 transition-all">+ Nuevo Drone</button>
        </header>
      ...
      <section className="space-y-6">
        <header className="flex justify-between items-end border-b border-slate-200 pb-4 text-left">
          ...
          <button onClick={() => setActivePanel('battery')} className="bg-[#1A202C] text-white px-6 py-2.5 rounded-xl font-black text-[10px] uppercase shadow-lg hover:bg-slate-800 transition-all">+ Nueva Batería</button>
        </header>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {batteries.map(bat => <BatteryCard key={bat.id} battery={bat} onEdit={(b) => setEditingBattery(b)} />)}
        </div>
      </section>
