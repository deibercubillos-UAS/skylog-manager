    ...
    <div className="flex h-screen overflow-hidden bg-[#f8f6f6] font-display text-left">
      ...
      <aside className="w-64 bg-[#1A202C] flex flex-col h-full border-r border-slate-700 shrink-0 shadow-2xl">
        <div className="p-6 flex items-center gap-3 border-b border-white/5 mb-4">
          ...
          <div className="text-left">
            <h1 className="text-white text-xl font-black tracking-tighter leading-none">BitaFly</h1>
            <p className="text-[#ec5b13] text-[8px] font-black uppercase tracking-widest mt-1">Plan {userPlan.toUpperCase()}</p>
          </div>
        </div>
            ...
            return (
              <Link key={item.href} href={item.href}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all ${
                  isActive ? 'bg-[#ec5b13]/10 border-l-4 border-[#ec5b13] text-[#ec5b13]' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
                }`}
              >
        ...
        <div className="p-4 border-t border-slate-800 bg-[#141a26]">
          ...
          <button onClick={handleLogout} className="w-full py-2 bg-slate-800 hover:bg-red-900/20 text-slate-300 hover:text-red-400 text-[10px] font-black rounded-lg uppercase tracking-widest transition-all">
            Cerrar Sesión
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0 z-10">
          ...
          <Link href="/dashboard/logbook/new" className="bg-[#ec5b13] hover:bg-orange-600 text-white px-5 py-2.5 rounded-xl text-[10px] font-black shadow-lg shadow-orange-500/20 uppercase tracking-widest transition-all hover:scale-105">
            Nuevo Vuelo
          </Link>
        </header>
