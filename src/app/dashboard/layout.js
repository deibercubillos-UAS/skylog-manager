...
export default function DashboardLayout({ children }) {
  ...
  const [profile, setProfile] = useState({ full_name: 'Cargando...', role: 'Operador' });
  ...
  // LOGICA DE BACKEND: Consumimos nuestra propia API de perfil
  useEffect(() => {
    async function loadProfile() {
      try {
        ...
        console.error("Error al conectar con la API de perfil");
      }
    }
    loadProfile();
  }, []);
  ...
  // Filtrado de menú según el rol que devolvió el backend
      ...
      <aside className="w-64 bg-[#1A202C] flex flex-col h-full border-r border-slate-700 shrink-0">
        <div className="p-6 flex items-center gap-3 border-b border-white/5 mb-4">
          ...
          <div className="text-left overflow-hidden">
            ...
            <p className="text-[#ec5b13] text-[8px] font-black uppercase tracking-widest truncate">Plan {userPlan.toUpperCase()}</p>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="flex items-center gap-3 mb-4 p-2 text-left bg-white/5 rounded-xl border border-white/5">
            <div className="size-10 rounded-full bg-slate-700 flex items-center justify-center text-white font-black">
          ...
          <button onClick={handleLogout} className="w-full py-2 bg-slate-800 text-slate-300 text-[10px] font-black rounded-lg uppercase">
            Cerrar Sesión
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0">
          <div className="flex-1 text-left">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Aviation Management System</span>
          </div>
          <Link href="/dashboard/logbook/new" className="bg-[#ec5b13] text-white px-4 py-2 rounded-xl text-[10px] font-black shadow-lg uppercase tracking-widest hover:scale-105 transition-transform">
            Nuevo Vuelo
          </Link>
        </header>
