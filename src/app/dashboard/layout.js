"use client";
import { useState, useEffect } from "react"; // IMPORTACIÓN CRÍTICA
...
export default function DashboardLayout({ children }) {
  ...
  const filteredMenuItems = menuItems.filter(item => {
    ...
    if (userRole === 'gerente_sms') return ['Dashboard', 'Análisis SORA', 'Personalizar Checklist', 'Reportes PDF'].includes(item.name);
    return true; // Por defecto pilotos ven todo lo básico
  });
      ...
      <aside className="w-64 bg-[#1A202C] flex flex-col h-full border-r border-slate-700 shrink-0">
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="flex items-center gap-3 mb-4 p-2 text-left">
            <div className="size-10 rounded-full bg-slate-700 overflow-hidden border border-slate-600 shrink-0 flex items-center justify-center text-white font-black">
          ...
          <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-slate-800 text-slate-300 text-[10px] font-black rounded-lg uppercase">
            Cerrar Sesión
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0">
          <div className="flex-1 text-left">
             <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">SkyLog Manager System</span>
          </div>
          <Link href="/dashboard/logbook/new" className="bg-[#ec5b13] text-white px-4 py-2 rounded-xl text-[10px] font-black shadow-lg uppercase tracking-widest">
            Nuevo Vuelo
          </Link>
        </header>
