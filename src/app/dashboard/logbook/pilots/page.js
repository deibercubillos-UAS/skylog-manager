        ...
        <div className="max-w-6xl mx-auto space-y-10 text-left animate-in fade-in duration-500 pb-20">
            <header className="flex flex-col md:flex-row justify-between items-center gap-6">
                ...
                <div className="w-full md:w-72 space-y-1 text-left">
                    <label className="text-[9px] font-black uppercase text-slate-400 ml-2">Filtrar por Tripulante</label>
                    <select 
                                ...
                                return (
                                    <tr key={v.id} className="hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-5">
                                            ...
                                            <p className="text-[9px] font-mono text-slate-400 uppercase">S/N: {v.aircraft?.serial_number}</p>
                                        </td>
                                        <td className="px-6 py-5">
                                            <p className="text-xs font-bold text-slate-700">{v.pilots?.name}</p>
                                            <p className="text-[9px] font-bold text-slate-400 uppercase">CIPU: {v.pilots?.cipu_number}</p>
                                        </td>
                                        <td className="px-6 py-5">
                                            <span className={`px-2 py-1 rounded text-[9px] font-black uppercase ${isExpired ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>
            ...
            {selectedPilotId && !filterLoading && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-bottom-4">
                    <div className="bg-[#1A202C] p-8 rounded-[2.5rem] text-white flex justify-between items-center">
                        <div>
                            <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Total Horas Acumuladas</p>
                    ...
                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 flex justify-between items-center text-left">
                        <div>
                            <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Vuelos Registrados</p>
