...
export default function DashboardPage() {
    ...
    <div className="flex justify-between items-start mb-10">
        <div>
            <h3 className="text-[10px] font-black uppercase text-slate-400 tracking-[0.3em]">Historial de Misiones</h3>
            <p className="text-xs font-bold text-slate-900 mt-1">Últimos {data?.stats?.totalFlights} vuelos registrados</p>
        </div>
        <span className="text-[9px] font-black bg-orange-100 px-2 py-1 rounded text-orange-600 uppercase">Monitor Activo</span>
    </div>
    
    <div className="flex-1 flex items-end justify-around gap-2 px-2 border-b border-slate-100 pb-2">
        {data?.chart?.map((m, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-3 group">
                {/* BARRA SIMPLIFICADA: Si hay vuelo (count > 0), la barra sube */}
                <div 
                    className={`w-full max-w-[35px] rounded-t-lg transition-all duration-700 ${m.count > 0 ? 'bg-orange-500 h-full' : 'bg-slate-50 h-2'}`}
                    style={{ height: m.count > 0 ? '80%' : '8px' }}
                ></div>
