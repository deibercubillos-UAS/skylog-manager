...
export default function ManageSubscriptionPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (user) {
        // Intentamos cargar el perfil para la UI, pero no bloquearemos el pago por esto
        const res = await fetch(`/api/user/profile?userId=${user.id}`);
        const data = await res.json();
        if (!data.error) {
          setProfile(data);
        } else {
          // Si no hay perfil en la tabla, creamos un objeto temporal con lo que tenemos de Auth
          setProfile({ id: user.id, email: user.email, subscription_plan: 'piloto' });
        }
      } else {
  ...
  const handleUpgrade = async (planName) => {
    ...
    // Usamos los datos de 'user' (Auth) que son 100% seguros
        ...
        <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 flex flex-col justify-between shadow-sm relative overflow-hidden">
          <div className="relative z-10 text-left">
            ...
            <p className="text-xs text-slate-500 mt-4 font-medium leading-relaxed">
              Gestiona tus límites de equipo y herramientas de seguridad.
            </p>
          </div>
      ...
      <footer className="bg-[#1A202C] rounded-[2.5rem] p-10 text-white flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl relative overflow-hidden">
        <div className="text-left relative z-10">
          <div className="flex items-center gap-3 mb-2 text-[#ec5b13]">
            ...
            <h4 className="text-lg font-black uppercase tracking-tight">¿Necesitas ayuda?</h4>
          </div>
          <p className="text-slate-400 text-sm font-medium max-w-xl leading-relaxed">
            Nuestro equipo técnico está disponible para ayudarte con tu facturación.
          </p>
        </div>
        <Link href="/#contacto" className="bg-white text-[#1A202C] px-10 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-[#ec5b13] hover:text-white transition-all shadow-lg">
          Contactar Soporte
        </Link>
      </footer>
    </div>
    ...
    <div className={`p-8 rounded-[2.5rem] border-2 bg-white flex flex-col justify-between transition-all duration-500 ${isActive ? 'border-[#ec5b13] bg-orange-50/20 ring-8 ring-orange-500/5 shadow-inner scale-105' : 'border-slate-100 hover:border-slate-300'}`}>
      <div className="text-left">
        <div className="flex justify-between items-start mb-6">
          ...
          {recommended && !isActive && <span className="bg-slate-900 text-white text-[8px] font-black px-3 py-1 rounded-full uppercase tracking-widest shadow-lg">Popular</span>}
        </div>
      ...
      {!isActive ? (
        <button onClick={onAction} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-[#ec5b13] transition-all shadow-xl active:scale-95">Suscribirme</button>
      ) : (
