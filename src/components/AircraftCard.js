...
export default function AircraftCard({ aircraft, onEdit, onDelete }) {
  ...
  const LIMIT_DAYS = 182; // Aprox 6 meses

  // 1. CÁLCULO POR HORAS
  const currentHours = parseFloat(aircraft.total_hours || 0);
  const lastMaintHours = parseFloat(aircraft.last_maintenance_hours || 0);
  const hoursSinceMaint = currentHours - lastMaintHours;
  const hourProgress = (hoursSinceMaint / LIMIT_HOURS) * 100;

  // 2. CÁLCULO POR TIEMPO
  const lastMaintDate = aircraft.last_maintenance_date ? new Date(aircraft.last_maintenance_date) : new Date(aircraft.created_at);
  ...
  // 3. DETERMINAR EL MÁS CRÍTICO (El porcentaje más alto)
  const finalProgress = Math.min(100, Math.max(hourProgress, timeProgress));
  const isTimeCritical = timeProgress > hourProgress;

  // 4. LÓGICA DE COLORES (Alerta al 90% / falta el 10%)
  let barColor = "bg-emerald-500";
  let statusText = "Sistemas OK";
  
  if (finalProgress >= 75) {
    barColor = "bg-orange-500";
    statusText = "Mantenimiento Próximo";
  }
  if (finalProgress >= 90) {
    barColor = "bg-red-600";
    statusText = isTimeCritical ? "Límite 6 meses alcanzado" : "Límite 200h alcanzado";
  }
      ...
      <div className="flex-1 p-5 md:p-6 flex flex-col justify-between">
        <div className="flex justify-between items-start">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => onDelete(aircraft.id)} className="size-8 rounded-xl bg-red-50 flex items-center justify-center text-red-400 hover:bg-red-600 hover:text-white transition-all"><span className="material-symbols-outlined text-lg">delete</span></button>
          </div>
        </div>
        ...
        <div className="space-y-3 mt-4">
          <div className="flex justify-between items-end">
            <div className="text-left">
                <p className={`text-[9px] font-black uppercase tracking-tighter ${finalProgress >= 90 ? 'text-red-600 animate-pulse' : 'text-slate-400'}`}>
                    {statusText}
                </p>
