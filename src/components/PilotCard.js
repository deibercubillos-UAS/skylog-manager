...
export default function PilotCard({ pilot }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm flex group hover:shadow-md transition-all text-left h-36">
      ...
      {/* SECCIÓN DERECHA: INFORMACIÓN Y DOCUMENTOS */}
      <div className="flex-1 p-5 flex flex-col justify-between">
        ...
        <div className="flex justify-between items-start">
          <div className="overflow-hidden">
            ...
            <p className="text-[#ec5b13] text-[9px] font-black uppercase tracking-[0.15em] mt-0.5">{pilot.pilot_role || 'Piloto de Mando'}</p>
          </div>

          {/* ICONOS DE DOCUMENTOS DESCARGABLES */}
          <div className="flex gap-2 shrink-0 ml-4">
            {pilot.certificate_url && (
              <a 
                href={pilot.certificate_url} 
                target="_blank" 
                rel="noreferrer" 
                title="Ver Licencia de Piloto"
                className="size-8 rounded-lg bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-400 hover:text-[#ec5b13] hover:border-[#ec5b13] transition-all"
              >
            ...
            {!pilot.certificate_url && !pilot.medical_url && (
              <span className="size-8 rounded-lg border border-slate-100 flex items-center justify-center text-slate-200" title="Sin documentos cargados">
