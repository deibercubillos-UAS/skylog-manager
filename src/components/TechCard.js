  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200 p-5 flex items-center gap-4 group hover:shadow-md transition-all text-left">
      ...
      <div className="flex-1 min-w-0">
        <h4 className="font-black text-slate-800 uppercase text-[11px] truncate leading-none">
            {item.brand} {item.model}
        </h4>
        <p className="text-[9px] font-mono text-slate-400 font-bold uppercase mt-1">S/N: {item.serial_number || 'N/A'}</p>
        <div className="mt-2">
            <span className="px-2 py-0.5 bg-purple-50 text-purple-600 rounded text-[8px] font-black uppercase">
      ...
      <div className="flex flex-col gap-1">
        <button onClick={() => onEdit(item)} className="p-1 text-slate-300 hover:text-orange-600 transition-colors">
            <span className="material-symbols-outlined text-lg">edit</span>
        </button>
        <button onClick={() => onDelete(item.id)} className="p-1 text-slate-200 hover:text-red-500 transition-colors">
